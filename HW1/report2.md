...
## 申論及開發報告
在本次實作中，探討二元搜尋樹（BST）在隨機資料輸入下的結構特性與效能表現。BST 的操作效率與樹的高度密切相關，而樹高又高度依賴於資料的插入順序，因此採用隨機數作為輸入，以觀察其平均情況下的狀況。  
在樹的建立過程中，透過隨機插入 n 個節點，使得整體結構趨近於「隨機樹」。結果顯示，樹高與 $\log_2 n$ 呈現近似線性關係，其比值 $height / \log_2 n$ 大致維持在 2 左右。這代表在平均情況下，BST 並不會嚴重偏斜，而是具有接近平衡樹的特性。  
然而，BST 的缺點是，當輸入資料為遞增或遞減序列時，樹會退化為類似鏈結串列的結構，此時樹高將達到 $O(n)$，導致搜尋、插入與刪除操作的效率大幅下降。  
在刪除操作的實作中，必須選擇適當的替代節點以維持 BST 的排序性質。採用「中序後繼」，該節點必然大於左子樹所有節點且小於右子樹其他節點，能確保替換後仍符合 BST 定義。若隨意選擇節點替代，將會破壞整體結構，導致搜尋結果錯誤。
